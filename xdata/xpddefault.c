...
/* Patch of Posting Performance Data to ActiveMQ */
#include <time.h>
#include "stomp.h"

apr_pool_t *xpddefault_perfdata_apr_pool;
stomp_connection *xpddefault_perfdata_activemq_connection;
stomp_frame xpddefault_perfdata_write_frame;
int xpddefault_perfdata_activemq_on = 0;
char *xpddefault_perfdata_activemq_host = NULL;
int xpddefault_perfdata_activemq_port = 9090;
char *xpddefault_perfdata_activemq_user = NULL;
char *xpddefault_perfdata_activemq_password = NULL;
char *xpddefault_perfdata_activemq_primary = NULL;
int xpddefault_perfdata_activemq_is_reconnecting = 0;
/* End Patch of Posting Performance Data to ActiveMQ */
...
/* Patch of Posting Performance Data to ActiveMQ */
int warn(int exitCode, const char *message, apr_status_t reason) {
	char msgbuf[80];

	apr_strerror(reason, msgbuf, sizeof(msgbuf));
	fprintf(stderr, "%s: %s (%d)\n", message, msgbuf, reason);
	msgbuf[sizeof(msgbuf)-1] = '\x0';
	logit(NSLOG_RUNTIME_WARNING, TRUE, msgbuf);

	if (!strcmp(msgbuf, "Broken pipe") || !strcmp(msgbuf, "Bad file descriptor")) {
		logit(NSLOG_RUNTIME_WARNING, TRUE, "Reconnecting ActiveMQ");
		connect_activemq();
	}
...
int connect_activemq(void) {
	apr_status_t rc;
	char temp_buffer[MAX_INPUT_BUFFER];
	stomp_frame frame;
	stomp_frame *read_frame;

	if (xpddefault_perfdata_activemq_is_reconnecting == 0) {
		xpddefault_perfdata_activemq_is_reconnecting = 1;

		rc = apr_initialize();
		rc == APR_SUCCESS || warn(-2, "ActiveMQ: Could not initialize", rc);

		rc == apr_pool_create(&xpddefault_perfdata_apr_pool, NULL);
		rc == APR_SUCCESS || warn(-2, "ActiveMQ: Could not allocate pool", rc);

		// Connect
		snprintf(temp_buffer, sizeof(temp_buffer) - 1, "Connecting to %s:%d\n", xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port);
		logit(NSLOG_CONFIG_ERROR, TRUE, temp_buffer);
		rc = stomp_connect( &xpddefault_perfdata_activemq_connection, xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port, xpddefault_perfdata_apr_pool);
		if (rc != APR_SUCCESS) {
			xpddefault_perfdata_activemq_is_reconnecting = 0;
			snprintf(temp_buffer, sizeof(temp_buffer) - 1, "Could not connect.Will try next sample.\n", xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port);
			logit(NSLOG_CONFIG_ERROR, TRUE, temp_buffer);
			return OK;
		}

		// Login
		snprintf(temp_buffer, sizeof(temp_buffer) - 1, "Logining to ActiveMQ\n", xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port);
		logit(NSLOG_CONFIG_ERROR, TRUE, temp_buffer);
		frame.command = "CONNECT";
		frame.headers = apr_hash_make(xpddefault_perfdata_apr_pool);
		apr_hash_set(frame.headers, "login", APR_HASH_KEY_STRING, xpddefault_perfdata_activemq_user);
		apr_hash_set(frame.headers, "passcode", APR_HASH_KEY_STRING, xpddefault_perfdata_activemq_password);
		frame.body = NULL;
		frame.body_length = -1;
		rc = stomp_write(xpddefault_perfdata_activemq_connection, &frame, xpddefault_perfdata_apr_pool);
		rc == APR_SUCCESS || warn(-2, "Could not send frame", rc);

		snprintf(temp_buffer, sizeof(temp_buffer) - 1, "ActiveMQ ReadReponse\n",xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port);
		logit(NSLOG_CONFIG_ERROR, TRUE, temp_buffer);
		rc = stomp_read(xpddefault_perfdata_activemq_connection, &read_frame, xpddefault_perfdata_apr_pool);
		rc == APR_SUCCESS || warn(-2, "Could not read frame", rc);
		fprintf(stdout, "%s.", read_frame->command);

		// Login complete, setup a reusable write frame
		snprintf(temp_buffer,sizeof(temp_buffer)-1,"ActiveMQ Connected\n",xpddefault_perfdata_activemq_host, xpddefault_perfdata_activemq_port);
		logit(NSLOG_CONFIG_ERROR, TRUE, temp_buffer);

		xpddefault_perfdata_write_frame.command = "SEND";
		xpddefault_perfdata_write_frame.headers = apr_hash_make(xpddefault_perfdata_apr_pool);
		apr_hash_set(xpddefault_perfdata_write_frame.headers, "destination", APR_HASH_KEY_STRING, xpddefault_perfdata_activemq_primary);
		apr_hash_set(xpddefault_perfdata_write_frame.headers, "persistent", APR_HASH_KEY_STRING, "true");
...
/* Patch of Posting Performance Data to ActiveMQ */
	else if(!strcmp(varname, "perfdata_activemq_on")) {
		strip(varvalue);
		xpddefault_perfdata_activemq_on=atoi(varvalue);
	}
	else if(!strcmp(varname,"perfdata_activemq_host"))
		xpddefault_perfdata_activemq_host=strdup(varvalue);
	else if(!strcmp(varname,"perfdata_activemq_port")) {
		strip(varvalue);
		xpddefault_perfdata_activemq_port=atoi(varvalue);
	}
	else if(!strcmp(varname,"perfdata_activemq_user"))
		xpddefault_perfdata_activemq_user=strdup(varvalue);
	else if(!strcmp(varname,"perfdata_activemq_password"))
		xpddefault_perfdata_activemq_password=strdup(varvalue);
	else if(!strcmp(varname,"perfdata_activemq_primary"))
		xpddefault_perfdata_activemq_primary=strdup(varvalue);
/* End Patch of Posting Performance Data to ActiveMQ */
...
/* Patch of Posting Performance Data to ActiveMQ */
	/* Connect to ActiveMQ if Enabled */
	if (xpddefault_perfdata_activemq_on == 1) {
		connect_activemq();
	}
/* End Patch of Posting Performance Data to ActiveMQ */
...
/* Patch of Posting Performance Data to ActiveMQ */
	if (xpddefault_perfdata_activemq_on == 1) {
		int e;
		xpddefault_perfdata_write_frame.body = processed_output;
		xpddefault_perfdata_write_frame.body_length = -1;
		rc = stomp_write(xpddefault_perfdata_activemq_connection, &xpddefault_perfdata_write_frame, xpddefault_perfdata_apr_pool);
		rc == APR_SUCCESS || warn(-2, "Could not send frame", rc);
	} else {
